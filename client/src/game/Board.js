...
export default function Board(props) {
  const { player, start, validation, history, modes, settings, completed } =
    props;
  ...
  return (
    <section className={`board ${props.className}`}>
      {player.map((row, rowIndex) => {
        return (
          <div key={`row_${rowIndex}`} className="board__row">
            {row.map((cell, cellIndex) => {
              ...
              let readonlyColor = userSettings.readonly.color,
              ...
              if (validation) {
                ...
                hasDuplicates =
                  (showRowTips &&
                    validation[`row_${rowIndex}`].duplicates.length > 0) ||
                  (showColTips &&
                    validation[`column_${cellIndex}`].duplicates.length > 0) ||
                  (showSubTips &&
                    validation[`subgrid_${subgridIndex}`].duplicates.length >
                      0);

                hasRemaining =
                  (showRowTips &&
                    validation[`row_${rowIndex}`].remaining.length > 0) ||
                  (showColTips &&
              ...
              if (startValue !== 0) {
                return (
                  <div
                    key={`cell_${cellIndex}`}
                    style={{ color: readonlyColor }}
                    id={`r${rowIndex}_c${cellIndex}`}
                    onKeyDown={(e) => arrowKeys(e, rowIndex, cellIndex)}
                    tabIndex="0"
                    className={classnames(
                      "board__cell board__cell--readonly",
                      `text_family-${readonlyFamily}`,
                      `text_weight-${readonlyWeight}`,
                      `text_style-${readonlyStyle}`,
                      { "board__cell--missing": showTips && hasRemaining },
                      {
                        "board__cell--duplicates":
                          showTips && hasDuplicates && hasRemaining,
                      },
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      ...
                      id={`r${rowIndex}_c${cellIndex}`}
                      style={{
                        color: thisMode === "guess" ? guessColor : defaultColor,
                      }}
                      className={classnames(
                        "board__input",
