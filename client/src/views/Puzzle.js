...
import OpenEye from "../img/open-eye.svg?react";
import CloseEye from "../img/close-eye.svg?react";

import Reset from "../img/reset.svg?react";
...
class Puzzle extends Component {
        ...
        if (isAuthenticated) {
          ...
          axios
            ...
            .then((userRes) => {
              ...
              const index = user.puzzles.findIndex(
                (obj) => obj.id === puzzleId,
              );
              ...
              if (index !== -1) {
                ...
                if (player.modes.length === 0)
                  player.modes = cloneDeep(mode_dictionary);
              }
              // if new, add to players list of puzzles
              else {
                // add this puzzle to user's list
                axios
                  .post("/users/id/" + userId + "/addPuzzle", player)
                  .then((saveRes) => {
              ...
              this.setState(
                ...
                () => {
                  ...
                  this.setState({
                    validation: this.validateEntireGrid(
                      history[current_history].state,
                    ),
                  });
                },
              );
  ...
  handleGrid = (e, rowIndex, cellIndex) => {
    ...
    const updateHistory = cloneDeep(
      currentHistory.slice(0, this.state.current_history + 1),
    );
    ...
    if (updateModes[posIndex].mode !== this.state.mode)
      updateModes[posIndex].mode = this.state.mode;
  ...
  changeHistory = (position, index) => {
    ...
    const confirm = window.confirm(
      `Are you sure you want to mass undo to the ${readPos} move? This action cannot be undone.`,
    );
  ...
  saveProgress = () => {
    axios
      .patch(
        "/users/id/" + this.props.auth.user.id + "/updatePuzzle",
        this.state.player,
      )
      .then((res) => {
        this.props.setCanLogout(true);
  ...
  resetPuzzle = () => {
    const confirm = window.confirm(
      "Are you sure you want to reset this puzzle? This action cannot be undone.",
    );
  ...
  checkAnswer = () => {
    if (this.searchValidation()) {
      ...
      this.setState({
        errors:
          "Not quite there! Double check your answer for missing numbers or duplicates.",
      });
    }
  };
  ...
  abandon = () => {
    const confirm = window.confirm(
      "Are you sure you want to abandon this puzzle? This action cannot be undone.",
    );
    ...
    if (confirm) {
      ...
      axios
        .patch("/users/id/" + this.props.auth.user.id + "/pullPuzzle", puzzle)
        .then((res) => {
      ...
      return (
        <div className="page">
          <div className="title-group">
            ...
            <IconName
              role="img"
              aria-label={`This puzzle is ${puzzle.difficulty} difficulty`}
              title={`This puzzle is ${puzzle.difficulty}`}
              width="26"
              height="26"
          ...
          <div className="view-puzzle">
            <div className="view-puzzle__subgrids">
              {sections.map((button) => {
                ...
                return (
                  <div
                    key={`btn-subgrid_${button}`}
                    className={classnames(
                      "validation validation_style-sub",
                      { "validation_color-default": !thisValidation.tip },
                      {
                        "validation_color-visible":
                          thisValidation.tip &&
                          thisValidation.duplicates.length === 0,
                      },
                      {
                      ...
                      {
                        "validation_color-complete":
                          thisValidation.tip &&
                          thisValidation.remaining.length === 0,
                      },
                    )}
                  >
                    <button
                      ...
                      onClick={(e) => this.toggleTip("subgrid", button)}
                      title="Press to toggle tips for this quadrant"
                    >
                      {sections.map((square) => {
                        return (
                          <div
                            key={`subgrid_square_${square}`}
                            className={classnames("validation__square", {
                    ...
                    {thisValidation.tip &&
                      thisValidation.remaining.length > 0 && (
                        <div className="validation__tip validation__tip--vertical">
                          {thisValidation.remaining.map((value, index) => (
                            <span key={`subgrid_${button}_${index}`}>
                              {value}
                            </span>
                          ))}
                        </div>
            ...
            <div className="view-puzzle__main">
              <Board
                start={puzzle.start}
                player={history[current_history].state}
                update={(e, rowIndex, cellIndex) =>
                  this.handleGrid(e, rowIndex, cellIndex)
                }
                changeHistory={(e, position, index) =>
                  this.changeHistory(e, position, index)
                }
                    ...
                    {thisValidation.tip &&
                      thisValidation.remaining.length > 0 && (
                        <div className="validation__tip validation__tip--horizontal">
                          {thisValidation.remaining.map((value, index) => (
                            <span key={`row_${button}_${index}`}>{value}</span>
                          ))}
                        </div>
                      )}
                  </div>
                  ...
                  >
                    ...
                    {thisValidation.tip &&
                      thisValidation.remaining.length > 0 && (
                        <div className="validation__tip validation__tip--vertical">
                          {thisValidation.remaining.map((value, index) => (
                            <span key={`column_${button}_${index}`}>
                              {value}
                            </span>
                          ))}
                        </div>
            ...
            <div className="view-puzzle__actions">
              {!player.completed && isAuthenticated && (
                <button
                  type="button"
                  className="button button_style-solid button_style-solid--default"
              ...
              >
                <div className="button__layout button__layout--icon-left">
                  ...
                  <span className="button__text">
                    {player.completed ? "Replay" : "Restart"}
                  </span>
                </div>
              </button>
              ...
              {!player.completed && (
                <>
                  ...
                  <div className="view-puzzle__mark-mode">
                    ...
                    <button
                      type="button"
                      onClick={(e) => this.toggleMode("guess")}
                      title="Set color to guess"
                      style={{ background: settings.guess.color }}
                      className={classnames(
                        "view-puzzle__mark-button view-puzzle__mark-button--guess",
                  ...
                  <div>
                    {current_history > 0 ? (
                      ...
                      <button
                        type="button"
                        className="button button_style-solid button--disabled"
                      >
                        Undo
                      </button>
                    )}
