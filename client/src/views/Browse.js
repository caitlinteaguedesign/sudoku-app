...
import Add from "../img/add.svg?react";
...
class Browse extends Component {
    ...
    if (isAuthenticated) {
      ...
      if (result) {
        ...
        for (const puz in puzzles) {
          ...
          const userVersion = userPuzzles.findIndex(
            (puz) => puz.id === thisPuzzle._id,
          );
  ...
  getAddedPuzzle() {
    ...
    return new URLSearchParams(location.search).get("added") === "true"
      ? true
      : false;
  }
      ...
      {easy.length > 0 && (
        <section className="section">
          <div className="section-title-icon">
            <Easy
              role="img"
              aria-label="these are easy puzzles"
              width="26"
              height="26"
      ...
      {medium.length > 0 && (
        <section className="section">
          <div className="section-title-icon">
            <Medium
              role="img"
              aria-label="these are medium puzzles"
              width="26"
              height="26"
              className="section-title-icon__icon"
            />
      ...
      {hard.length > 0 && (
        <section className="section">
          <div className="section-title-icon">
            <Hard
              role="img"
              aria-label="these are hard puzzles"
              width="26"
              height="26"
              className="section-title-icon__icon"
            />
      ...
      {expert.length > 0 && (
        <section className="section">
          <div className="section-title-icon">
            <Expert
              role="img"
              aria-label="these are expert puzzles"
              width="26"
              height="26"
              className="section-title-icon__icon"
            />
...
function listPuzzles(puzzles) {
  return (
    <ul className="puzzle-list">{puzzles.map((obj) => singlePuzzle(obj))}</ul>
  );
}
  ...
  return (
    <li key={puzzle._id} className="puzzle-list__item">
      <IconName
        role="img"
        aria-label={
          `This puzzle has a difficulty of ${puzzle.difficulty}` +
          userLabelModifier
        }
        width="52"
        height="52"
        className={classnames(
          "puzzle-list__icon",
          {
            "puzzle-list__icon--completed":
              "isCompleted" in puzzle && puzzle.isCompleted,
          },
          {
            "puzzle-list__icon--inprogress":
              "isCompleted" in puzzle && !puzzle.isCompleted,
          },
      ...
      <span className="puzzle-list__date">
        ...
        <span className="text_bold">
          {formatDate(puzzle.date_created, "MM/DD/YYYY")}
        </span>
      </span>
    </li>
  );
